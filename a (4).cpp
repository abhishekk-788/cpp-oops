...
signed main()
{
    ...
    fo(i, 0, num_int)
    {
        ...
        {
            output_file << i << "\n"
                        << intersections[i].in.size() << "\n";

            int dur_each = floor(duration / intersections[i].in.size());

            random_shuffle(all(intersections[i].in));

            fo(j, 0, intersections[i].in.size() - 1)
                    output_file
                << st_rev_map.at(intersections[i].in[j]) << " " << dur_each << "\n";

            int dur = dur_each + (duration % dur_each);

            output_file << st_rev_map.at(intersections[i].in[intersections[i].in.size() - 1]) << " " << dur << "\n";
        }
    }
